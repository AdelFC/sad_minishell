...
/*   Updated: 2025/06/13 12:31:23 by afodil-c         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	split_assignment_quoted(const char *raw_value,
		char **assign_key, char **assign_val, int *was_quoted)
{
	int		i;
	int		len;
	char	quote;
    ...
    while (cur)
    {
        ...
        if (cur->type == RAW_OPERATOR)
        {
            ...
            if (!raw.value)
            {
                ...
                free_tokens(*tokens_out); // Libère les tokens déjà créés
                return (ERROR);
            }
            raw.was_quoted = 0;
        }
        ...
        {
            if (split_assignment_quoted(cur->value, &raw.key,
                    &raw.val, &raw.was_quoted) == SUCCESS)
            {
            ...
            else
            {
                ...
                char *tmp = remove_outer_quotes(raw.value, &raw.was_quoted);
                if (!tmp)
                {
                    free(raw.value);
                    free_ppraw_utils(&raw);
